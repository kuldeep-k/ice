	...
	tinymce.create('tinymce.plugins.IcePlugin', {
		...
		init: function(ed, url) {
			...
			ed.onPostRender.add(function(mgr) {
                ...
                if(self.isTracking)
                {
                    if (ed.controlManager.get('bold')) {
                        ed.controlManager.get('bold').settings.cmd='ice_trackbold';
                    }
                }	
			...
			ed.addCommand('initializeice', function(editor) {
				...
				self.insertTag = self.deleteTag = 'span';
				    //console.log(self.insertTag);
            ...
            // Added by kuldeep			
			ed.addCommand('ice_trackbold', function() {
			    return changeEditor.pluginsManager.plugins['IceTrackFormatPlugin'].handleFormatting('bold');
			});
